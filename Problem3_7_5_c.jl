...
sol3 = solve(ODEProblem(reSys, u1, tend1, params))

p3 = plot(sol1, ylims=(0.0, 0.8),
     title="full vs reduced -> S is small compare to Km",
...
sol4 = solve(ODEProblem(reSys, u2, tend2, params))

p4 = plot(sol2, ylims=(0.0, 8.0),
     title="full vs reduced -> S is large compare to Km",
     xlabel="Time (arbitrary units)",
     ylabel="Concentration (arbitrary units)",
     labels=["S1 " "S2 " "S3 "], ls=:dash)
